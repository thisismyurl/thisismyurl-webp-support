...
class TIMU_WEBP_Support {
    ...
    /**
     * Handle Vortops API key save form submission.
     *
     * @return void
     */
    public static function handle_vortops_save() {
        check_admin_referer( 'timu_webp_vortops_save', 'timu_webp_vortops_nonce' );
        if ( ! current_user_can( 'manage_options' ) ) {
            wp_die( esc_html__( 'Unauthorized.', 'thisismyurl-webp-support' ) );
        }
        ...
        wp_safe_redirect( add_query_arg(
            array( 'page' => 'webp-optimizer', 'tab' => 'settings', 'vortops-saved' => '1' ),
            admin_url( 'tools.php' )
        ) );
        exit;
    }
                                                ...
                                                ?>
                                                    <label style="display:block;margin-bottom:4px;">
                                                        ...
                                                        <?php if ( $needs_avif && ! $avif_ok ) : ?>
                                                            <em style="color:#d63638;">&#8212; <?php esc_html_e( 'AVIF unavailable: server lacks Imagick with libheif.', 'thisismyurl-webp-support' ); ?></em>
                                                            <?php if ( TIMU_Vortops_Client::is_connected() ) : ?>
                                                            <em style="color:#00a32a;"> <?php esc_html_e( 'Vortops cloud AVIF is connected and will be used.', 'thisismyurl-webp-support' ); ?></em>
                                                            <?php else : ?>
                                                            <em style="color:#646970;"> <?php esc_html_e( 'Connect Vortops below for cloud AVIF conversion.', 'thisismyurl-webp-support' ); ?></em>
                                                            <?php endif; ?>
                                                        <?php endif; ?>
                                                    </label>
                                                <?php endforeach; ?>
